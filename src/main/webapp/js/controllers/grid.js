/**
 * Created by sophia.wang on 17/3/24.
 */
'use strict';
...
app.filter('positionFilter', function(){
    return function(input){
        switch (input){
            case 'Integer Corporation':    return 'aa-' + input;
            case 'In PC':   return 'bb-' + input;
...
app.controller('GridDemoCtrl', function($scope,$modal,$log,$timeout) {
    ...
    $scope.colArr = [
        ...
        },
        {
            field: "position",
            displayName: '职位',
            cellTemplate: '<div>{{ row.entity.position | positionFilter}}</div>'
        },
        {
            field: "office",
            displayName: '公司地址'
    ...
    $scope.delData = function (size) {
        var modalInstance = $modal.open({
            backdrop: false,
            templateUrl: 'table_ui_grid_delete',
            controller: 'delTCtrl',
            size: size,
            resolve: {
